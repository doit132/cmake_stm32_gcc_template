...
/* define compiler specific symbols */
	...
	#define __INLINE                                                                           \
		inline /*!< inline keyword for IAR Compiler. Only avaiable in High optimization    \
			  mode! */
...
uint32_t __STREXB(uint8_t value, uint8_t *addr)
{
	...
	__ASM volatile("strexb %0, %2, [%1]" : "=&r"(result) : "r"(addr), "r"(value)); // GCC 修改
...
uint32_t __STREXH(uint16_t value, uint16_t *addr)
{
	...
	__ASM volatile("strexh %0, %2, [%1]" : "=&r"(result) : "r"(addr), "r"(value)); // GCC 修改
	return (result);
}

...
typedef struct {
	...
	#define HASH_FLAG_DINNE                                                                    \
		HASH_CR_DINNE /*!< DIN not empty : The input buffer contains at least one word of  \
				 data                 */
...
ErrorStatus HMAC_MD5(uint8_t *Key, uint32_t Keylen, uint8_t *Input, uint32_t Ilen,
		     uint8_t Output[16]);

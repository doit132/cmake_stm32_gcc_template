...
extern "C" {
	...
	/* Exported types ------------------------------------------------------------*/
		...
		#define IS_CAN_ALL_PERIPH(PERIPH)                                                  \
			(((PERIPH) == CAN1) || ((PERIPH) == CAN2) || ((PERIPH) == CAN3))
	#else
...
typedef struct {
	uint16_t CAN_FilterIdHigh; /*!< Specifies the filter identification number (MSBs for a
				      32-bit configuration, first one for a 16-bit configuration).
					      This parameter can be a value between 0x0000 and
				      0xFFFF */
	...
	uint16_t CAN_FilterMaskIdHigh; /*!< Specifies the filter mask number or identification
					  ...
					  configuration, first one for a 16-bit configuration). This
	...
	uint16_t CAN_FilterMaskIdLow; /*!< Specifies the filter mask number or identification
					 ...
					 configuration, second one for a 16-bit configuration). This
					 parameter can be a value between 0x0000 and 0xFFFF */

	uint16_t CAN_FilterFIFOAssignment; /*!< Specifies the FIFO (0 or 1) which will be assigned
					      to the filter. This parameter can be a value of @ref
					      CAN_filter_FIFO */

	uint8_t CAN_FilterNumber; /*!< Specifies the filter which will be initialized. It ranges
				     from 0 to 13. */
 ...
 */
	...
	#define IS_CAN_OPERATING_MODE(MODE)                                                        \
		(((MODE) == CAN_OperatingMode_Initialization) ||                                   \
		 ((MODE) == CAN_OperatingMode_Normal) || ((MODE) == CAN_OperatingMode_Sleep))
/**
 * @}
 */

...
typedef struct {
	...
	#define IS_ADC_MODE(MODE)                                                                  \
		(((MODE) == ADC_Mode_Independent) ||                                               \
		 ...
		 ((MODE) == ADC_DualMode_InjecSimult) || ((MODE) == ADC_DualMode_RegSimult) ||     \
		 ((MODE) == ADC_DualMode_Interl) || ((MODE) == ADC_DualMode_AlterTrig) ||          \
	...
	#define ADC_DMAAccessMode_1                                                                \
		((uint32_t)0x00004000) /* DMA mode 1 enabled (2 / 3 half-words one by one - 1 then \
					  2 then 3)*/
	#define ADC_DMAAccessMode_2                                                                \
		((uint32_t)0x00008000) /* DMA mode 2 enabled (2 / 3 half-words by pairs - 2&1 then \
					  1&3 then 3&2)*/
	#define ADC_DMAAccessMode_3                                                                \
		((uint32_t)0x0000C000) /* DMA mode 3 enabled (2 / 3 bytes by pairs - 2&1 then 1&3  \
					  then 3&2) */

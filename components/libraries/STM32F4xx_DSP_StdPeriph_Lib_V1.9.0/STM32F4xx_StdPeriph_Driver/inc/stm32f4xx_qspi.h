...
typedef struct {
	...
	uint32_t QSPI_ComConfig_DDRMode; /* Specifies the Double Data Rate Mode
					  This parameter can be a value of @ref
					  QSPI_ComConfig_DoubleDataRateMode*/
...
typedef struct {
	...
	uint32_t QSPI_FSize;   /* Specifies the Flash Size.
			       QSPI_FSize+1 is effectively the number of address bits required to
			       address the flash memory.   The flash capacity can be up to 4GB
			       (addressed using 32 bits) in indirect mode, but the   addressable space
			       in memory-mapped mode is limited to 512MB   This parameter can be a
			       number between 0x00 and 0x1F */
	...
	#define IS_QSPI_ABSIZE(ABSIZE)                                                             \
		(((ABSIZE) == QSPI_ComConfig_ABSize_8bit) ||                                       \
	...
	#define IS_QSPI_ADSIZE(ADSIZE)                                                             \
		(((ADSIZE) == QSPI_ComConfig_ADSize_8bit) ||                                       \

 ...
 */
	...
	#define IS_I2C_GET_IT(IT)                                                                  \
		(((IT) == I2C_IT_SMBALERT) || ((IT) == I2C_IT_TIMEOUT) ||                          \
		 ...
		 ((IT) == I2C_IT_RXNE) || ((IT) == I2C_IT_STOPF) || ((IT) == I2C_IT_ADD10) ||      \
 ...
 */
	 ...
	 * I2C_EVENT_SLAVE_XXX_SECONDADDRESS_MATCHED (where XXX could be TRANSMITTER or RECEIVER)
	 * are set.
	 *
	 ...
	 *      the end of the byte transmission. The two events I2C_EVENT_SLAVE_BYTE_TRANSMITTED
	 * and I2C_EVENT_SLAVE_BYTE_TRANSMITTING are similar. The second one can optionally be used
	 * when the user software doesn't guarantee the EV3 is managed before the current byte end
	 * of transfer.
	 *    - EV3_2: When the master sends a NACK in order to tell slave that data transmission
	 *      shall end (before sending the STOP condition). In this case slave has to stop
	 * sending data bytes and expect a Stop condition on the bus.
	 *

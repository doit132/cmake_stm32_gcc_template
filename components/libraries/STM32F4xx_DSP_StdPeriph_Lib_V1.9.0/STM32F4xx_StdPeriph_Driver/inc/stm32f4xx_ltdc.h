...
typedef struct {
	...
	uint32_t LTDC_HorizontalSync; /*!< configures the number of Horizontal synchronization
						 width. This parameter must range from 0x000 to
					 0xFFF. */

	uint32_t LTDC_VerticalSync; /*!< configures the number of Vertical synchronization
						 height. This parameter must range from 0x000 to
				       0x7FF. */

	uint32_t LTDC_AccumulatedHBP; /*!< configures the accumulated horizontal back porch width.
						 This parameter must range from LTDC_HorizontalSync
					 to 0xFFF. */

	uint32_t LTDC_AccumulatedVBP; /*!< configures the accumulated vertical back porch height.
						 This parameter must range from LTDC_VerticalSync to
					 0x7FF. */

	uint32_t LTDC_AccumulatedActiveW; /*!< configures the accumulated active width. This
					     parameter must range from LTDC_AccumulatedHBP to 0xFFF.
					   */

	uint32_t LTDC_AccumulatedActiveH; /*!< configures the accumulated active height. This
					     parameter must range from LTDC_AccumulatedVBP to 0x7FF.
					   */

	uint32_t
		LTDC_TotalWidth; /*!< configures the total width. This parameter
						must range from LTDC_AccumulatedActiveW to 0xFFF. */

	uint32_t
		LTDC_TotalHeigh; /*!< configures the total height. This parameter
 ...
 */
	...
	#define LTDC_VSPolarity_AL                                                                 \
		((uint32_t)0x00000000)		  /*!< Vertical Synchronization is active low. */
	#define LTDC_VSPolarity_AH LTDC_GCR_VSPOL /*!< Vertical Synchronization is active high. */

	#define IS_LTDC_VSPOL(VSPOL)                                                               \
		(((VSPOL) == LTDC_VSPolarity_AL) || ((VSPOL) == LTDC_VSPolarity_AH))
	/**
	 * @}
	 */
	...
	#define IS_LTDC_DEPOL(DEPOL)                                                               \
		(((DEPOL) == LTDC_VSPolarity_AL) || ((DEPOL) == LTDC_DEPolarity_AH))
	/**
	 * @}
	 */
	...
	#define IS_LTDC_Pixelformat(Pixelformat)                                                   \
		(((Pixelformat) == LTDC_Pixelformat_ARGB8888) ||                                   \
		 ((Pixelformat) == LTDC_Pixelformat_RGB888) ||                                     \
		 ((Pixelformat) == LTDC_Pixelformat_RGB565) ||                                     \
		 ((Pixelformat) == LTDC_Pixelformat_ARGB1555) ||                                   \
		 ((Pixelformat) == LTDC_Pixelformat_ARGB4444) ||                                   \
		 ((Pixelformat) == LTDC_Pixelformat_L8) ||                                         \
		 ((Pixelformat) == LTDC_Pixelformat_AL44) ||                                       \
	...
	#define IS_LTDC_BlendingFactor1(BlendingFactor1)                                           \
		(((BlendingFactor1) == LTDC_BlendingFactor1_CA) ||                                 \
		 ((BlendingFactor1) == LTDC_BlendingFactor1_PAxCA))
	/**
	 * @}
	 */
	...
	#define IS_LTDC_BlendingFactor2(BlendingFactor2)                                           \
		(((BlendingFactor2) == LTDC_BlendingFactor2_CA) ||                                 \
		 ((BlendingFactor2) == LTDC_BlendingFactor2_PAxCA))
	/**
	 * @}
	 */

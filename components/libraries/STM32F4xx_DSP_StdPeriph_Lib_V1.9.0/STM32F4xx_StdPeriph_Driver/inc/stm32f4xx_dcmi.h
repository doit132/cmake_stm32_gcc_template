...
typedef struct {
	...
	uint16_t DCMI_HSPolarity; /*!< Specifies the Horizontal synchronization polarity: High or
				     Low. This parameter can be a value of @ref DCMI_HSYNC_Polarity
				   */

	uint16_t DCMI_CaptureRate; /*!< Specifies the frequency of frame capture: All, 1/2 or 1/4.
				       This parameter can be a value of @ref DCMI_Capture_Rate */

	uint16_t DCMI_ExtendedDataMode; /*!< Specifies the data width: 8-bit, 10-bit, 12-bit or
				       14-bit. This parameter can be a value of @ref
 ...
 */
typedef struct {
	uint16_t DCMI_VerticalStartLine; /*!< Specifies the Vertical start line count from which the
					    image capture will start. This parameter can be a value
					    between 0x00 and 0x1FFF */

	uint16_t DCMI_HorizontalOffsetCount; /*!< Specifies the number of pixel clocks to count
					      before starting a capture. This parameter can be a
					      value between 0x00 and 0x3FFF */

	uint16_t DCMI_VerticalLineCount; /*!< Specifies the number of lines to be captured from the
					    starting point. This parameter can be a value between
					    0x00 and 0x3FFF */

	uint16_t DCMI_CaptureCount; /*!< Specifies the number of pixel clocks to be captured from
				       the starting point on the same line. This parameter can be a
				       value between 0x00 and 0x3FFF */
...
typedef struct {
	...
	#define DCMI_CaptureMode_Continuous                                                        \
		((uint16_t)0x0000) /*!< The received data are transferred continuously             \
							       into the destination memory through \
				      the DMA */
	#define DCMI_CaptureMode_SnapShot                                                          \
		((uint16_t)0x0002) /*!< Once activated, the interface waits for the start of       \
							       frame and then transfers a single   \
				      frame through the DMA */
	#define IS_DCMI_CAPTURE_MODE(MODE)                                                         \
		(((MODE) == DCMI_CaptureMode_Continuous) || ((MODE) == DCMI_CaptureMode_SnapShot))
	/**
	 * @}
	 */
	...
	#define DCMI_SynchroMode_Hardware                                                          \
		((uint16_t)0x0000) /*!< Hardware synchronization data capture (frame/line          \
				      start/stop) is synchronized with the HSYNC/VSYNC signals */
	#define DCMI_SynchroMode_Embedded                                                          \
		((uint16_t)0x0010) /*!< Embedded synchronization data capture is synchronized with \
							     synchronization codes embedded in the \
				      data flow */
	#define IS_DCMI_SYNCHRO(MODE)                                                              \
		(((MODE) == DCMI_SynchroMode_Hardware) || ((MODE) == DCMI_SynchroMode_Embedded))
	/**
	 * @}
	 */
	...
	#define IS_DCMI_EXTENDED_DATA(DATA)                                                        \
		(((DATA) == DCMI_ExtendedDataMode_8b) || ((DATA) == DCMI_ExtendedDataMode_10b) ||  \
		 ((DATA) == DCMI_ExtendedDataMode_12b) || ((DATA) == DCMI_ExtendedDataMode_14b))
	/**
	 * @}
	 */
	...
	#define IS_DCMI_GET_IT(IT)                                                                 \
		(((IT) == DCMI_IT_FRAME) || ((IT) == DCMI_IT_OVF) || ((IT) == DCMI_IT_ERR) ||      \
		 ((IT) == DCMI_IT_VSYNC) || ((IT) == DCMI_IT_LINE))
	/**
	 * @}
	 */

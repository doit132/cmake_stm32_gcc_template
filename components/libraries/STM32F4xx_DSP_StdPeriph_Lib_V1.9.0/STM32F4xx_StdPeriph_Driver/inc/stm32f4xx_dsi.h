...
typedef struct {
	...
	uint32_t HorizontalSyncActive; /*!< Horizontal synchronism active duration (in lane byte
					  clock cycles) */

	uint32_t HorizontalBackPorch; /*!< Horizontal back-porch duration (in lane byte clock
					 cycles)         */
	...
	uint32_t LPVACTLargestPacketSize; /*!< The size, in bytes, of the low power largest packet
					     that can fit in a line during VACT region */
	...
	uint32_t
		LPVerticalSyncActiveEnable; /*!< Low-power vertical sync active enable
...
typedef struct {
	...
	uint32_t CommandSize; /*!< Maximum allowed size for an LTDC write memory command, measured
				 in pixels. This parameter can be any value between 0x00 and 0xFFFF
			       */

	uint32_t TearingEffectSource; /*!< Tearing effect source
				       This parameter can be any value of @ref
				       DSI_TearingEffectSource   */
 ...
 */
typedef struct {
	uint32_t ClockLaneHS2LPTime; /*!< The maximum time that the D-PHY clock lane takes to go
					from high-speed to low-power transmission */

	uint32_t ClockLaneLP2HSTime; /*!< The maximum time that the D-PHY clock lane takes to go
					from low-power to high-speed transmission */

	uint32_t DataLaneHS2LPTime; /*!< The maximum time that the D-PHY data lanes takes to go from
				       high-speed to low-power transmission */

	uint32_t DataLaneLP2HSTime; /*!< The maximum time that the D-PHY data lanes takes to go from
				       low-power to high-speed transmission */

	uint32_t DataLaneMaxReadTime; /*!< The maximum time required to perform a read command */

	uint32_t StopWaitTime; /*!< The minimum wait period to request a High-Speed transmission
...
typedef struct {
	...
	#define DSI_FLOW_CONTROL_ALL                                                               \
		(DSI_FLOW_CONTROL_CRC_RX | DSI_FLOW_CONTROL_ECC_RX | DSI_FLOW_CONTROL_BTA |        \
		 DSI_FLOW_CONTROL_EOTP_RX | DSI_FLOW_CONTROL_EOTP_TX)
	 ...
	 */
	#define DSI_RGB565                                                                         \
		((uint32_t)0x00000000) /*!< The values 0x00000001 and 0x00000002 can also be used  \
	...
	#define IS_DSI_PLL_IDF(IDF)                                                                \
		(((IDF) == DSI_PLL_IN_DIV1) || ((IDF) == DSI_PLL_IN_DIV2) ||                       \
		 ((IDF) == DSI_PLL_IN_DIV3) || ((IDF) == DSI_PLL_IN_DIV4) ||                       \
		 ((IDF) == DSI_PLL_IN_DIV5) || ((IDF) == DSI_PLL_IN_DIV6) ||                       \
		 ((IDF) == DSI_PLL_IN_DIV7))
	/**
	 * @}
	 */
	...
	#define IS_DSI_PLL_ODF(ODF)                                                                \
		(((ODF) == DSI_PLL_OUT_DIV1) || ((ODF) == DSI_PLL_OUT_DIV2) ||                     \
		 ((ODF) == DSI_PLL_OUT_DIV4) || ((ODF) == DSI_PLL_OUT_DIV8))
	...
	#define DSI_DCS_SHORT_PKT_WRITE_P0                                                         \
		((uint32_t)0x00000005) /*!< DCS short write, no parameters      */
	#define DSI_DCS_SHORT_PKT_WRITE_P1                                                         \
		((uint32_t)0x00000015) /*!< DCS short write, one parameter      */
	#define DSI_GEN_SHORT_PKT_WRITE_P0                                                         \
		((uint32_t)0x00000003) /*!< Generic short write, no parameters  */
	#define DSI_GEN_SHORT_PKT_WRITE_P1                                                         \
		((uint32_t)0x00000013) /*!< Generic short write, one parameter  */
	#define DSI_GEN_SHORT_PKT_WRITE_P2                                                         \
		((uint32_t)0x00000023) /*!< Generic short write, two parameters */
	#define IS_DSI_SHORT_WRITE_PACKET_TYPE(MODE)                                               \
		(((MODE) == DSI_DCS_SHORT_PKT_WRITE_P0) ||                                         \
		 ...
		 ((MODE) == DSI_GEN_SHORT_PKT_WRITE_P1) || ((MODE) == DSI_GEN_SHORT_PKT_WRITE_P2))
	/**
	 * @}
	 */
	...
	#define IS_DSI_LONG_WRITE_PACKET_TYPE(MODE)                                                \
		(((MODE) == DSI_DCS_LONG_PKT_WRITE) || ((MODE) == DSI_GEN_LONG_PKT_WRITE))
	/**
	 * @}
	 */
	...
	#define DSI_DCS_SHORT_PKT_READ                                                             \
		((uint32_t)0x00000006) /*!< DCS short read                     */
	#define DSI_GEN_SHORT_PKT_READ_P0                                                          \
		((uint32_t)0x00000004) /*!< Generic short read, no parameters  */
	#define DSI_GEN_SHORT_PKT_READ_P1                                                          \
		((uint32_t)0x00000014) /*!< Generic short read, one parameter  */
	#define DSI_GEN_SHORT_PKT_READ_P2                                                          \
		((uint32_t)0x00000024) /*!< Generic short read, two parameters */
	...
	#define IS_DSI_PHY_TIMING(Timing)                                                          \
		(((Timing) == DSI_TCLK_POST) || ((Timing) == DSI_TLPX_CLK) ||                      \
		 ((Timing) == DSI_THS_EXIT) || ((Timing) == DSI_TLPX_DATA) ||                      \
		 ((Timing) == DSI_THS_ZERO) || ((Timing) == DSI_THS_TRAIL) ||                      \
		 ((Timing) == DSI_THS_PREPARE) || ((Timing) == DSI_TCLK_ZERO) ||                   \
...
void DSI_StructInit(DSI_InitTypeDef *DSI_InitStruct,
		    DSI_HOST_TimeoutTypeDef *DSI_HOST_TimeoutInitStruct);
...
void DSI_ShortWrite(DSI_TypeDef *DSIx, uint32_t ChannelID, uint32_t Mode, uint32_t Param1,
		    uint32_t Param2);
void DSI_LongWrite(DSI_TypeDef *DSIx, uint32_t ChannelID, uint32_t Mode, uint32_t NbParams,
		   uint32_t Param1, uint8_t *ParametersTable);
void DSI_Read(DSI_TypeDef *DSIx, uint32_t ChannelNbr, uint8_t *Array, uint32_t Size, uint32_t Mode,
...
void DSI_SetSlewRateAndDelayTuning(DSI_TypeDef *DSIx, uint32_t CommDelay, uint32_t Lane,
				   uint32_t Value);
void DSI_SetLowPowerRXFilter(DSI_TypeDef *DSIx, uint32_t Frequency);
void DSI_SetSDD(DSI_TypeDef *DSIx, FunctionalState State);
void DSI_SetLanePinsConfiguration(DSI_TypeDef *DSIx, uint32_t CustomLane, uint32_t Lane,

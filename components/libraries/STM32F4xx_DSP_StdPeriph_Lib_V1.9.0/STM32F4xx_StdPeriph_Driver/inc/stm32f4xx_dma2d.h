...
typedef struct {
	...
	uint32_t DMA2D_OutputBlue; /*!< configures the blue value of the output image.
						 ...
						 - from 0x00 to 0xFF if ARGB8888 color mode is
				      slected
						 - from 0x00 to 0xFF if RGB888 color mode is slected
						 - from 0x00 to 0x1F if RGB565 color mode is slected
						 - from 0x00 to 0x1F if ARGB1555 color mode is
				      slected
						 - from 0x00 to 0x0F if ARGB4444 color mode is
				      slected  */

	uint32_t
		DMA2D_OutputGreen; /*!< configures the green value of the output image.
						This parameter must range:
						- from 0x00 to 0xFF if ARGB8888 color mode is
				      selected
	...
	uint32_t DMA2D_OutputAlpha; /*!< configures the alpha channel of the output color.
						 ...
						 - from 0x00 to 0xFF if ARGB8888 color mode is
				       selected
						 - from 0x00 to 0x01 if ARGB1555 color mode is
				       selected
						 - from 0x00 to 0x0F if ARGB4444 color mode is
				       selected  */
	...
	uint32_t DMA2D_OutputOffset; /*!< Specifies the Offset value. This parameter must be range
					from 0x0000 to 0x3FFF. */

	uint32_t
		DMA2D_NumberOfLine; /*!< Configures the number of line of the area to be transfered.
						This parameter must range from 0x0000 to 0xFFFF */

	uint32_t DMA2D_PixelPerLine; /*!< Configures the number pixel per line of the area to be
					transferred. This parameter must range from 0x0000 to 0x3FFF
				      */
} DMA2D_InitTypeDef;

typedef struct {
	uint32_t DMA2D_FGMA; /*!< configures the DMA2D foreground memory address.
						 This parameter must be range from 0x00000000 to
				0xFFFFFFFF. */
	...
	uint32_t DMA2D_FGCMAR; /*!< Configures the DMA2D foreground CLUT memory address.
						 This parameter must range from 0x00000000 to
				  0xFFFFFFFF. */
 ...
 */
	...
	#define IS_DMA2D_FGCM(FGCM)                                                                \
		(((FGCM) == CM_ARGB8888) || ((FGCM) == CM_RGB888) || ((FGCM) == CM_RGB565) ||      \
		 ((FGCM) == CM_ARGB1555) || ((FGCM) == CM_ARGB4444) || ((FGCM) == CM_L8) ||        \
		 ((FGCM) == CM_AL44) || ((FGCM) == CM_AL88) || ((FGCM) == CM_L4) ||                \
		 ((FGCM) == CM_A8) || ((FGCM) == CM_A4))

	#define IS_DMA2D_BGCM(BGCM)                                                                \
		(((BGCM) == CM_ARGB8888) || ((BGCM) == CM_RGB888) || ((BGCM) == CM_RGB565) ||      \
		 ((BGCM) == CM_ARGB1555) || ((BGCM) == CM_ARGB4444) || ((BGCM) == CM_L8) ||        \
		 ((BGCM) == CM_AL44) || ((BGCM) == CM_AL88) || ((BGCM) == CM_L4) ||                \
		 ((BGCM) == CM_A8) || ((BGCM) == CM_A4))
 ...
 */
	...
	#define IS_DMA2D_BG_CLUT_CM(BG_CLUT_CM)                                                    \
		(((BG_CLUT_CM) == CLUT_CM_ARGB8888) || ((BG_CLUT_CM) == CLUT_CM_RGB888))
 ...
 */
	...
	#define IS_DMA2D_IT(IT)                                                                    \
		(((IT) == DMA2D_IT_CTC) || ((IT) == DMA2D_IT_CAE) || ((IT) == DMA2D_IT_TW) ||      \
		 ((IT) == DMA2D_IT_TC) || ((IT) == DMA2D_IT_TE) || ((IT) == DMA2D_IT_CE))

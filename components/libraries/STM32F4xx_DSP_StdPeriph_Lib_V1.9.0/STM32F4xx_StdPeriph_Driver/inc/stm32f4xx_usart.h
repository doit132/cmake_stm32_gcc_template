...
typedef struct {
	uint32_t USART_BaudRate; /*!< This member configures the USART communication baud rate.
				    ...
				    "oversampling by 8 mode" configuration bit in the CR1 register.
				  */

	uint16_t USART_WordLength; /*!< Specifies the number of data bits transmitted or received in
				      a frame. This parameter can be a value of @ref
				      USART_Word_Length */
	...
	uint16_t USART_Parity; /*!< Specifies the parity mode.
				  ...
				  inserted at the MSB position of the transmitted data (9th bit when
						 the word length is set to 9 data bits; 8th bit when
				  the word length is set to 8 data bits). */

	uint16_t USART_Mode; /*!< Specifies whether the Receive or Transmit mode is enabled or
				disabled. This parameter can be a value of @ref USART_Mode */

	uint16_t USART_HardwareFlowControl; /*!< Specifies wether the hardware flow control mode is
...
typedef struct {
	...
	uint16_t USART_LastBit; /*!< Specifies whether the clock pulse corresponding to the last
 ...
 */
	...
	#define IS_USART_ALL_PERIPH(PERIPH)                                                        \
		(((PERIPH) == USART1) || ((PERIPH) == USART2) || ((PERIPH) == USART3) ||           \
		 ...
		 ((PERIPH) == UART7) || ((PERIPH) == UART8) || ((PERIPH) == UART9) ||              \
		 ((PERIPH) == UART10))
 ...
 */
	...
	#define IS_USART_GET_IT(IT)                                                                \
		(((IT) == USART_IT_PE) || ((IT) == USART_IT_TXE) || ((IT) == USART_IT_TC) ||       \
		 ((IT) == USART_IT_RXNE) || ((IT) == USART_IT_IDLE) || ((IT) == USART_IT_LBD) ||   \
		 ((IT) == USART_IT_CTS) || ((IT) == USART_IT_ORE) || ((IT) == USART_IT_ORE_RX) ||  \
		 ((IT) == USART_IT_ORE_ER) || ((IT) == USART_IT_NE) || ((IT) == USART_IT_FE))

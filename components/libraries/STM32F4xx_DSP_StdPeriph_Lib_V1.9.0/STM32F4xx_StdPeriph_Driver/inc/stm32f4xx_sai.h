...
typedef struct {
	...
	uint32_t
		SAI_SynchroExt; /*!< Specifies SAI external output synchronization, this setup is
				   common for BlockA and BlockB This parameter can be a value of
				   @ref SAI_Block_SyncExt
					@note: If both audio blocks of same SAI are used, this
				   parameter has to be set to the same value for each audio block */

	uint32_t
		SAI_OUTDRIV; /*!< Specifies when SAI Block outputs are driven.
					This parameter can be a value of @ref SAI_Block_Output_Drive
					@note this value has to be set before enabling the audio
				block but after the audio block configuration. */

	uint32_t
		SAI_NoDivider; /*!< Specifies whether Master Clock will be divided or not.
					This parameter can be a value of @ref SAI_Block_NoDivider */

	uint32_t
		SAI_MasterDivider; /*!< Specifies SAI Block Master Clock Divider.
					@note the Master Clock Frequency is calculated accordingly
				      to the following formula : MCLK_x = SAI_CK_x/(MCKDIV[3:0]*2)*/
...
typedef struct {
	uint32_t SAI_FrameLength; /*!< Specifies the Frame Length, the number of SCK clocks
					 ...
					 @note If master Clock MCLK_x pin is declared as an output,
				     the frame length should be Aligned to a number equal to power
				     of 2 in order to keep in an audio frame, an integer number of
				     MCLK pulses by bit Clock.
					 @note this value is ignored when AC'97 or SPDIF protocols
				     are selected.*/

	uint32_t
		SAI_ActiveFrameLength; /*!< Specifies the Frame synchronization active level length.
					This Parameter specifies the length in number of bit clock
					(SCK + 1) of the active level of FS signal in audio frame.
...
typedef struct {
	uint32_t SAI_FirstBitOffset; /*!< Specifies the position of first data transfer bit in the
					slot. This parameter must be a number between 0 and 24.
					 @note this value is ignored when AC'97 or SPDIF protocols
					are selected.*/
	...
	uint32_t
		SAI_SlotActive; /*!< Specifies the slots in audio frame that will be activated.
	 ...
	 */
		...
		#define IS_SAI_BLOCK_PERIPH(PERIPH)                                                \
			(((PERIPH) == SAI1_Block_A) || ((PERIPH) == SAI1_Block_B) ||               \
			 ((PERIPH) == SAI2_Block_A) || ((PERIPH) == SAI2_Block_B))
	#endif /* STM32F446xx */

	#if defined(STM32F40_41xxx) || defined(STM32F427_437xx) || defined(STM32F429_439xx) ||     \
		defined(STM32F401xx) || defined(STM32F411xE) || defined(STM32F413_423xx) ||        \
		...
		#define IS_SAI_BLOCK_PERIPH(PERIPH)                                                \
		  ...
		  STM32F411xE || STM32F413_423xx || STM32F469_479xx */
	...
	#define IS_SAI_BLOCK_DATASIZE(DATASIZE)                                                    \
		(((DATASIZE) == SAI_DataSize_8b) || ((DATASIZE) == SAI_DataSize_10b) ||            \
		 ((DATASIZE) == SAI_DataSize_16b) || ((DATASIZE) == SAI_DataSize_20b) ||           \
	...
	#define IS_SAI_BLOCK_CONFIG_IT(IT)                                                         \
		(((IT) == SAI_IT_OVRUDR) || ((IT) == SAI_IT_MUTEDET) || ((IT) == SAI_IT_WCKCFG) || \
		 ((IT) == SAI_IT_FREQ) || ((IT) == SAI_IT_CNRDY) || ((IT) == SAI_IT_AFSDET) ||     \
		 ((IT) == SAI_IT_LFSDET))
	/**
	 * @}
	 */
	...
	#define IS_SAI_BLOCK_GET_FLAG(FLAG)                                                        \
		(((FLAG) == SAI_FLAG_OVRUDR) || ((FLAG) == SAI_FLAG_MUTEDET) ||                    \
		 ((FLAG) == SAI_FLAG_WCKCFG) || ((FLAG) == SAI_FLAG_FREQ) ||                       \
		 ((FLAG) == SAI_FLAG_CNRDY) || ((FLAG) == SAI_FLAG_AFSDET) ||                      \
	...
	#define IS_SAI_BLOCK_FIFO_STATUS(STATUS)                                                   \
		(((STATUS) == SAI_FIFOStatus_Less1QuarterFull) ||                                  \
		 ...
		 ((STATUS) == SAI_FIFOStatus_Full) || ((STATUS) == SAI_FIFOStatus_Empty))
/**
 * @}
 */
...
void SAI_FlushFIFO(SAI_Block_TypeDef *SAI_Block_x);
		...
		defined(STM32F439xx) || defined(STM32F469_479xx) || defined(STM32F413_423xx) ||    \
		defined(STM32F446xx)
void SAI_BlockSynchroConfig(SAI_InitTypeDef *SAI_InitStruct, SAI_TypeDef *SAIx);
	#endif /* STM32F427xx || STM32F437xx || STM32F429xx || STM32F439xx || STM32F469_479xx ||   \
		  STM32F413_423xx || STM32F446xx */

			 ...
			 uint8_t *Output, uint8_t *AuthTAG)
{
	...
	if (Mode == MODE_ENCRYPT) /* AES encryption */
	{
		...
		if (HLength != 0) {
			...
			if (CRYP_GetCmdStatus() == DISABLE) {
				/* The CRYP peripheral clock is not enabled or the device doesn't
	    embed the CRYP peripheral (please check the device sales type. */
				return (ERROR);
			}
	   ...
	   embed the CRYP peripheral (please check the device sales type. */
			...
			for (loopcounter = 0; ((loopcounter < ILength) && (status != ERROR));
			     loopcounter += 16) {
				/* Wait until the IFEM flag is reset */
				while (CRYP_GetFlagStatus(CRYP_FLAG_IFEM) == RESET) {
				}
			 ...
			 uint32_t TAGSize)
{
	...
	if (headersize != 0) {
		/* Check that the associated data (or header) length is lower than 2^16 - 2^8 =
		 * 65536 - 256 = 65280 */
		if (headersize < 65280) {
		...
		if ((headersize % 16) != 0) {
			...
			for (loopcounter = headersize; loopcounter <= ((headersize / 16) + 1) * 16;
			     loopcounter++) {
	...
	/* Flags byte */
	blockb0[0] |= 0u | (((((uint8_t)TAGSize - 2) / 2) & 0x07) << 3) |
		      (((uint8_t)(15 - NonceSize) - 1) & 0x07);

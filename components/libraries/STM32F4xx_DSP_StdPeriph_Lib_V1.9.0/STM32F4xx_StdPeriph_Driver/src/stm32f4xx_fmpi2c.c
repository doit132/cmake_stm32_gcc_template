...
void FMPI2C_OwnAddress2Config(FMPI2C_TypeDef* FMPI2Cx, uint16_t Address, uint8_t Mask)
{
        ...
        /* Set FMPI2Cx SADD */
        tmpreg |= (uint32_t)(((uint32_t)Address & FMPI2C_OAR2_OA2) | (((uint32_t)Mask << 8) & FMPI2C_OAR2_OA2MSK));
                             ...
                             uint32_t        StartStopMode)
{
        ...
        tmpreg &= (uint32_t) ~((uint32_t)(FMPI2C_CR2_SADD | FMPI2C_CR2_NBYTES | FMPI2C_CR2_RELOAD | FMPI2C_CR2_AUTOEND
                                          | FMPI2C_CR2_RD_WRN | FMPI2C_CR2_START | FMPI2C_CR2_STOP));

        /* update tmpreg */
        tmpreg |= (uint32_t)(((uint32_t)Address & FMPI2C_CR2_SADD)
                             | (((uint32_t)Number_Bytes << 16) & FMPI2C_CR2_NBYTES) | (uint32_t)ReloadEndMode
                             | (uint32_t)StartStopMode);

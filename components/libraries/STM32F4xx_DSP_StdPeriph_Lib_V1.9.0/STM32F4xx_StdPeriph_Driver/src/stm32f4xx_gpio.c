 ...
 * @param  GPIOx: where x can be (A..K) to select the GPIO peripheral for STM32F405xx/407xx and
 * STM32F415xx/417xx devices x can be (A..I) to select the GPIO peripheral for STM32F42xxx/43xxx
 * devices. x can be (A, B, C, D and H) to select the GPIO peripheral for STM32F401xx devices.
 * @retval None
 */
 ...
 *            @arg GPIO_AF_TAMPER: Connect TAMPER pins (TAMPER_1 and TAMPER_2) to AF0 (default after
 * reset)
...
void GPIO_PinAFConfig(GPIO_TypeDef *GPIOx, uint16_t GPIO_PinSource, uint8_t GPIO_AF)
{
	...
	GPIOx->AFR[GPIO_PinSource >> 0x03] &=
		~((uint32_t)0xF << ((uint32_t)((uint32_t)GPIO_PinSource & (uint32_t)0x07) * 4));

 ...
 */
	...
	#define DSI_ERROR_ACK_MASK                                                                 \
		(DSI_ISR0_AE0 | DSI_ISR0_AE1 | DSI_ISR0_AE2 | DSI_ISR0_AE3 | DSI_ISR0_AE4 |        \
		 DSI_ISR0_AE5 | DSI_ISR0_AE6 | DSI_ISR0_AE7 | DSI_ISR0_AE8 | DSI_ISR0_AE9 |        \
		 DSI_ISR0_AE10 | DSI_ISR0_AE11 | DSI_ISR0_AE12 | DSI_ISR0_AE13 | DSI_ISR0_AE14 |   \
		 DSI_ISR0_AE15)
	#define DSI_ERROR_PHY_MASK                                                                 \
		(DSI_ISR0_PE0 | DSI_ISR0_PE1 | DSI_ISR0_PE2 | DSI_ISR0_PE3 | DSI_ISR0_PE4)
	...
	#define DSI_ERROR_GEN_MASK                                                                 \
		(DSI_ISR1_GCWRE | DSI_ISR1_GPWRE | DSI_ISR1_GPTXE | DSI_ISR1_GPRDE | DSI_ISR1_GPRXE)

	#define DSI_MAX_RETURN_PKT_SIZE                                                            \
		((uint32_t)0x00000037) /*!< Maximum return packet configuration */
/**
 * @}
 */
...
static void DSI_ConfigPacketHeader(DSI_TypeDef *DSIx, uint32_t ChannelID, uint32_t DataType,
...
void DSI_Init(DSI_TypeDef *DSIx, DSI_InitTypeDef *DSI_InitStruct, DSI_PLLInitTypeDef *PLLInit)
{
	...
	DSIx->WRPCR |=
		(((PLLInit->PLLNDIV) << 2) | ((PLLInit->PLLIDF) << 11) | ((PLLInit->PLLODF) << 16));
	...
	unitIntervalx4 = (4000000 * tempIDF * (1 << PLLInit->PLLODF)) /
...
void DSI_ConfigAdaptedCommandMode(DSI_TypeDef *DSIx, DSI_CmdCfgTypeDef *CmdCfg)
{
	...
	DSIx->WCFGR |= (CmdCfg->TearingEffectSource | CmdCfg->TearingEffectPolarity |
...
void DSI_ConfigCommand(DSI_TypeDef *DSIx, DSI_LPCmdTypeDef *LPCmd)
{
	...
	DSIx->CMCR &= ~(DSI_CMCR_GSW0TX | DSI_CMCR_GSW1TX | DSI_CMCR_GSW2TX | DSI_CMCR_GSR0TX |
	...
	DSIx->CMCR |=
		(LPCmd->LPGenShortWriteNoP | LPCmd->LPGenShortWriteOneP |
		 LPCmd->LPGenShortWriteTwoP | LPCmd->LPGenShortReadNoP | LPCmd->LPGenShortReadOneP |
		 LPCmd->LPGenShortReadTwoP | LPCmd->LPGenLongWrite | LPCmd->LPDcsShortWriteNoP |
		 LPCmd->LPDcsShortWriteOneP | LPCmd->LPDcsShortReadNoP | LPCmd->LPDcsLongWrite |
...
void DSI_ConfigPhyTimer(DSI_TypeDef *DSIx, DSI_PHY_TimerTypeDef *PhyTimers)
{
	...
	maxTime = (PhyTimers->ClockLaneLP2HSTime > PhyTimers->ClockLaneHS2LPTime)
			  ? PhyTimers->ClockLaneLP2HSTime
			  : PhyTimers->ClockLaneHS2LPTime;

	/* Clock lane timer configuration */
	/* In Automatic Clock Lane control mode, the DSI Host can turn off the clock lane between
     two High-Speed transmission. To do so, the DSI Host calculates the time required for the clock
     lane to change from HighSpeed to Low-Power and from Low-Power to High-Speed. This timings are
     configured by the HS2LP_TIME and LP2HS_TIME in the DSI Host Clock Lane Timer Configuration
     Register (DSI_CLTCR). But the DSI Host is not calculating LP2HS_TIME + HS2LP_TIME but 2 x
     HS2LP_TIME.

     Workaround : Configure HS2LP_TIME and LP2HS_TIME with the same value being the max of
     HS2LP_TIME or LP2HS_TIME.
  */
	...
	DSIx->DLTCR |= (PhyTimers->DataLaneMaxReadTime | ((PhyTimers->DataLaneLP2HSTime) << 16) |
			((PhyTimers->DataLaneHS2LPTime) << 24));
		    ...
		    uint32_t Param2)
{
	...
	DSI_ConfigPacketHeader(DSIx, ChannelID, Mode, Param1, Param2);
}
		   ...
		   uint32_t Param1, uint8_t *ParametersTable)
{
	...
	/* Set the DCS code hexadecimal on payload byte 1, and the other parameters on the write
	 * FIFO command*/
	while (uicounter < NbParams) {
		if (uicounter == 0x00) {
			DSIx->GPDR = (Param1 | ((uint32_t)(*(ParametersTable + uicounter)) << 8) |
	...
	DSI_ConfigPacketHeader(DSIx, ChannelID, Mode, ((NbParams + 1) & 0x00FF),
			       (((NbParams + 1) & 0xFF00) >> 8));
}
	      ...
	      uint32_t DCSCmd, uint8_t *ParametersTable)
{
	...
	if (Size > 2) {
		...
		DSI_ShortWrite(DSIx, ChannelNbr, DSI_MAX_RETURN_PKT_SIZE, ((Size)&0xFF),
			       (((Size) >> 8) & 0xFF));
	}
	...
	{
		DSI_ConfigPacketHeader(DSIx, ChannelNbr, Mode, ParametersTable[0],
				       ParametersTable[1]);
	}
...
void DSI_ExitULPMData(DSI_TypeDef *DSIx)
{
	...
	{
		while ((DSIx->PSR & (DSI_PSR_UAN0 | DSI_PSR_UAN1)) !=
...
void DSI_ExitULPM(DSI_TypeDef *DSIx)
{
	...
	/* Wait until all active lanes exit ULPM */
	if ((DSIx->PCONFR & DSI_PCONFR_NL) == DSI_ONE_DATA_LANE) {
		while ((DSIx->PSR & (DSI_PSR_UAN0 | DSI_PSR_UANC)) !=
		       (DSI_PSR_UAN0 | DSI_PSR_UANC)) {
		}
	} else /* DSI_TWO_DATA_LANES */
	{
		while ((DSIx->PSR & (DSI_PSR_UAN0 | DSI_PSR_UAN1 | DSI_PSR_UANC)) !=

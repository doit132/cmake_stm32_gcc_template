...
#define RCC_CFGR_OFFSET	   (RCC_OFFSET + 0x08)
	...
	#define RCC_CFGR_MCO1EN_BB                                                                 \
		(PERIPH_BB_BASE + (RCC_CFGR_OFFSET * 32) + (RCC_MCO1EN_BIT_NUMBER * 4))

	/* Alias word address of MCO2EN bit */
	#define RCC_MCO2EN_BIT_NUMBER 0x9
	#define RCC_CFGR_MCO2EN_BB                                                                 \
		(PERIPH_BB_BASE + (RCC_CFGR_OFFSET * 32) + (RCC_MCO2EN_BIT_NUMBER * 4))
#endif /* STM32F410xx */
...
@verbatim
 ...
 ===================================================================================
	...
	(++) The second output is used to generate the clock for the USB OTG FS (48 MHz),
	  ...
	  interface and LCD TFT controller available only for STM32F42xxx/43xxx/446xx/469xx/479xx
devices.
...
void RCC_DeInit(void)
{
	...
	/* Reset HSEON, CSSON, PLLON, PLLI2S and PLLSAI(STM32F42xxx/43xxx/446xx/469xx/479xx devices)
	 * bits */
	...
	defined(STM32F401xx) || defined(STM32F411xE) || defined(STM32F446xx) ||                    \
	defined(STM32F413_423xx) || defined(STM32F469_479xx)
	/* Reset PLLI2SCFGR register */
	RCC->PLLI2SCFGR = 0x20003000;
#endif /* STM32F40_41xxx || STM32F427_437xx || STM32F429_439xx || STM32F401xx || STM32F411xE ||    \
	  STM32F446xx || STM32F413_423xx || STM32F469_479xx */
	...
	*/
	...
	/* Disable Timers clock prescalers selection, only available for STM32F42/43xxx and
	 * STM32F413_423xx devices */
	RCC->DCKCFGR = 0x00000000;

#if defined(STM32F410xx) || defined(STM32F413_423xx)
	/* Disable LPTIM and FMPI2C clock prescalers selection, only available for STM32F410xx and
	 * STM32F413_423xx devices */
		   ...
		   uint32_t PLLQ)
{
	...
	RCC->PLLCFGR =
		PLLM | (PLLN << 6) | (((PLLP >> 1) - 1) << 16) | (RCC_PLLSource) | (PLLQ << 24);
}
#endif /* STM32F40_41xxx || STM32F427_437xx || STM32F429_439xx || STM32F401xx || STM32F411xE */
		      ...
		      uint32_t PLLI2SR)
{
	...
	RCC->PLLI2SCFGR = PLLI2SM | (PLLI2SN << 6) | (((PLLI2SP >> 1) - 1) << 16) |
			  (PLLI2SQ << 24) | (PLLI2SR << 28);
}
...
void RCC_PLLSAIConfig(uint32_t PLLSAIN, uint32_t PLLSAIP, uint32_t PLLSAIQ, uint32_t PLLSAIR)
{
	...
	RCC->PLLSAICFGR =
		(PLLSAIN << 6) | (((PLLSAIP >> 1) - 1) << 16) | (PLLSAIQ << 24) | (PLLSAIR << 28);
}
...
@verbatim
 ...
 +---------------|----------------|----------------|-----------------|-----------------+
      (#) For STM32F42xxx/43xxx/469xx/479xx devices, the maximum frequency of the SYSCLK and HCLK is
180 MHz, PCLK2 90 MHz and PCLK1 45 MHz. Depending on the device voltage range, the maximum frequency
should be adapted accordingly:
 ...
 +-------------------------------------------------------------------------------------+
      ...
      (#) For STM32F410xx/STM32F411xE devices, the maximum frequency of the SYSCLK and HCLK is 100
MHz, PCLK2 100 MHz and PCLK1 50 MHz. Depending on the device voltage range, the maximum frequency
should be adapted accordingly:
 +-------------------------------------------------------------------------------------+
 | Latency       |                HCLK clock frequency (MHz)                           |
 |               |---------------------------------------------------------------------|
 ...
 * (RCC_SYSCLKSource_PLLPCLK for STM32F446xx devices)
 *            @arg RCC_SYSCLKSource_PLLRCLK: PLL R selected as system clock source only for
 * STM32F412xG, STM32F413_423xx and STM32F446xx devices
 * @retval None
 */
 ...
 *              - 0x0C: PLL R used as system clock (only for STM32F412xG, STM32F413_423xx and
 * STM32F446xx devices)
 */
...
void RCC_RTCCLKConfig(uint32_t RCC_RTCCLKSource)
{
	...
	if ((RCC_RTCCLKSource & 0x00000300) ==
	    0x00000300) { /* If HSE is selected as RTC clock source, configure HSE division factor
			     for RTC clock */
		tmpreg = RCC->CFGR;
...
void RCC_AHB2PeriphClockCmd(uint32_t RCC_AHB2Periph, FunctionalState NewState)
{
	...
	defined(STM32F427_437xx) || defined(STM32F429_439xx) || defined(STM32F446xx) ||            \
	defined(STM32F469_479xx)
/**
 ...
 *           - RCC_AHB3Periph_FSMC or RCC_AHB3Periph_FMC
 * (STM32F412xG/STM32F413_423xx/STM32F429x/439x devices)
...
#endif /* STM32F40_41xxx || STM32F412xG || STM32F413_423xx || STM32F427_437xx || STM32F429_439xx   \
 ...
 *            @arg RCC_APB2Periph_SAI1:   SAI1 clock (STM32F42xxx/43xxx/446xx/469xx/479xx/413_423xx
 * devices)
 ...
 *            @arg RCC_AHB2Periph_CRYP:   CRYP clock
 *            @arg RCC_AHB2Periph_HASH:   HASH clock
 *            @arg RCC_AHB2Periph_RNG:    RNG clock for
 * STM32F40_41xxx/STM32F412xG/STM32F413_423xx/STM32F427_437xx/STM32F429_439xx/STM32F469_479xx
 * devices
 *            @arg RCC_AHB2Periph_OTG_FS: USB OTG FS clock
 * @param  NewState: new state of the specified peripheral reset.
 *          This parameter can be: ENABLE or DISABLE.
 ...
 *          This parameter must be:
 *           - RCC_AHB3Periph_FSMC or RCC_AHB3Periph_FMC (STM32F412xG, STM32F413_423xx and
 ...
 * @note This mode is only available for STM32F410xx/STM32F411xx/STM32F446xx/STM32F469_479xx
 * devices.
 ...
 *            @arg RCC_DSICLKSource_PLLR: PLL_R used as DSI byte lane clock source, used in case DSI
 * PLL and DSI-PHY are off (low power mode).
 * @retval None
 */
 ...
 *            @arg RCC_IT_PLLSAIRDY: PLLSAI ready interrupt (only for
 ...
 *            @arg RCC_FLAG_PLLSAIRDY: PLLSAI clock ready (only for
 * STM32F42xxx/43xxx/446xx/469xx/479xx devices)
 ...
 *            @arg RCC_IT_PLLRDY: main PLL ready interrupt
 *            @arg RCC_IT_PLLI2SRDY: PLLI2S ready interrupt
 *            @arg RCC_IT_PLLSAIRDY: PLLSAI clock ready interrupt (only for
 * STM32F42xxx/43xxx/446xx/469xx/479xx devices)

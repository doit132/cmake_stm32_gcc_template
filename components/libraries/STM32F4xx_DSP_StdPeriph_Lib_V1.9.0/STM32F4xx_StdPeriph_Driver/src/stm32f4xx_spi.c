...
void I2S_Init(SPI_TypeDef *SPIx, I2S_InitTypeDef *I2S_InitStruct)
{
	...
	/* If the requested audio frequency is not the default, compute the prescaler */
       ...
       in the project configuration or in the stm32f4xx_conf.h file */
		...
		/* Compute the Real divider depending on the MCLK output state, with a floating
		 * point */
		if (I2S_InitStruct->I2S_MCLKOutput == I2S_MCLKOutput_Enable) {
			/* MCLK output is enabled */
			tmp = (uint16_t)(((((i2sclk / 256) * 10) / I2S_InitStruct->I2S_AudioFreq)) +
					 5);
		} else {
	...
	if ((I2S_InitStruct->I2S_Standard == I2S_Standard_PCMShort) ||
	    (I2S_InitStruct->I2S_Standard == I2S_Standard_PCMLong)) {
 ...
 *** Interrupt Mode ***
 ...
 [..] In Interrupt Mode, the SPI communication can be managed by 3 interrupt sources
   ...
   (+) Pending Bits:
       ...
       (##) SPI_IT_CRCERR : to indicate if a CRC Calculation error occur (available in SPI mode
only)

/**
 ...
 Destination data formats, Circular or Normal mode, Stream Priority level, Source and Destination
      ...
      (#) Optionally, if the Circular mode is enabled, you can use the Double buffer mode by
 configuring the second Memory address and the first Memory to be used through the function
...
#define DMA_Stream0_IT_MASK                                                                        \
	(uint32_t)(DMA_LISR_FEIF0 | DMA_LISR_DMEIF0 | DMA_LISR_TEIF0 | DMA_LISR_HTIF0 |            \
		   DMA_LISR_TCIF0)
...
void DMA_Init(DMA_Stream_TypeDef *DMAy_Streamx, DMA_InitTypeDef *DMA_InitStruct)
{
	...
	tmpreg &= ((uint32_t) ~(DMA_SxCR_CHSEL | DMA_SxCR_MBURST | DMA_SxCR_PBURST | DMA_SxCR_PL |
				DMA_SxCR_MSIZE | DMA_SxCR_PSIZE | DMA_SxCR_MINC | DMA_SxCR_PINC |
				DMA_SxCR_CIRC | DMA_SxCR_DIR));
...
@verbatim
 ...
 ===============================================================================
    ...
    stopped) to modify on of the target Memories addresses or to check which Memory target is
currently used:
    ...
    To check this condition, it is recommended to use the function DMA_GetCurrentMemoryTarget()
which returns the index of the Memory target currently in use by the DMA Stream.
...
@verbatim
 ...
 ===============================================================================
    ...
    *** Interrupt Source ***
    ...
    [..]
    In this Mode it is advised to use the following functions:
      (+) void DMA_ITConfig(DMA_Stream_TypeDef* DMAy_Streamx, uint32_t DMA_IT, FunctionalState
NewState);

...
@verbatim
 ...
 [..]
 +-------------------------------------------------------------------------------------------------------------------+
 |               | voltage range  | voltage range  | voltage range   | voltage range   | voltage
range 2.7 V - 3.6 V | |               | 2.7 V - 3.6 V  | 2.4 V - 2.7 V  | 2.1 V - 2.4 V   | 1.8 V
- 2.1 V   | with External Vpp = 9V      |
 ...
 +-------------------------------------------------------------------------------------------------------------------+
	  ...
	  On STM32F42xxx/43xxx devices:
	   ...
	   (++) when VOS[1:0] = '0x10' Scale 2 mode, the maximum value of fHCLK is 144MHz if
OverDrive OFF and 168MHz if OverDrive ON.
	   (++) when VOS[1:0] = '0x11' Scale 1 mode, the maximum value of fHCLK is 168MHz if
OverDrive OFF and 180MHz if OverDrive ON.
	  [..]
 ...
 *
 * @note For STM32F405xx/407xx, STM32F415xx/417xx, STM32F401xx/411xE/STM32F412xG and STM32F413_423xx
 * devices this parameter can be a value between FLASH_Latency_0 and FLASH_Latency_7.
 *
 ...
 * SPRMOD = 1 and WRPi = 1 (default value), then the user sector i is read/write protected. In order
 * to avoid activation of PCROP Mode for undesired sectors, please follow the below safety sequence
 * :
 *         - Disable PCROP for all Sectors using FLASH_OB_PCROPConfig(OB_PCROP_Sector_All, DISABLE)
 * function for Bank1 or FLASH_OB_PCROP1Config(OB_PCROP_Sector_All, DISABLE) function for Bank2
 *         - Enable PCROP for the desired Sector i using FLASH_OB_PCROPConfig(Sector i, ENABLE)
 * function
 ...
 *            @arg OB_PcROP_Enable: nWRPi control the  read&write protection (PCROP) of respective
 * user sectors.
 * @retval None
 */
 ...
 *
 * @note   This function can be used only for STM32F42xxx/43xxx devices and STM32F401xx/411xE
 * devices.
 *
 * @param  None
 ...
 *            @arg FLASH_FLAG_RDERR: FLASH (PCROP) Read Protection error flag (STM32F42xx/43xxx and
 ...
 *            @arg FLASH_FLAG_PGPERR: FLASH Programming Parallelism error flag
 *            @arg FLASH_FLAG_PGSERR: FLASH Programming Sequence error flag
 *            @arg FLASH_FLAG_RDERR: FLASH Read Protection error flag (STM32F42xx/43xxx and
 * STM32F401xx/411xE devices)
 * @retval None
 */
